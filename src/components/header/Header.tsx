import { useCallback, useState } from "react";
import { NavLink } from "react-router-dom";
import { IoMdContact, IoMdHome } from "react-icons/io";
import { SiAmazonsimpleemailservice } from "react-icons/si";
import { FcAbout } from "react-icons/fc";
import { LuMenu, LuX } from "react-icons/lu"; // Importing icons for menu
import "./header.css";
import { useNavigate } from "react-router-dom";

const Header = () => {
  const [isOpen, setIsOpen] = useState(false);
  const navigate = useNavigate();
  const [isCopied, setCopied] = useState<boolean>(false);

  const navs = [
    { id: 1, name: "Home", slug: "/", icon: <IoMdHome /> },
    {
      id: 2,
      name: "Service",
      slug: "/service",
      icon: <SiAmazonsimpleemailservice />,
    },
    { id: 3, name: "Contact", slug: "/contact", icon: <IoMdContact /> },
    { id: 4, name: "About Us", slug: "/about-us", icon: <FcAbout /> },
  ];

  const copyText = useCallback(
    (data: string) => {
      setCopied(true);
      navigator.clipboard.writeText(data);
       setTimeout(() => {
        setCopied(false);
      },1000);
    },
    [isCopied]
  );

  return (
    <nav className="bg-gradient-to-r bg-[#0b2f2a] text-cyan-50 shadow-lg relative">
      <div className="container mx-auto flex justify-between items-center py-4">
        {/* Logo */}
        <div
          className="flex items-center"
          style={{ cursor: "pointer" }}
          onClick={() => navigate("/")}
        >
          <img
            src="data:image/png;base64,"
            alt="Logo"
            className="h-10 w-auto mr-3"
          />
          <span className="text-2xl font-bold">The fish</span>
        </div>

        {/* Contact Details */}
        <div className="hidden md:flex space-x-6 contact-header-div relative contact-main-div">
          <span
            className="hover:text-white transition duration-200 cursor-pointer item-to-copy"
            onClick={() => copyText("saaarock4646@gmail.com")}
          >
            Email: info@example.com
          </span>
          <span
            className="hover:text-white transition duration-200 cursor-pointer item-to-copy"
            onClick={() => copyText("9823464648")}
          >
            Phone: +123 456 7890
          </span>
          {isCopied ? (
            <div className="animate-message animate-message-block">Copied</div>
          ) : (
            <div className="animate-message">
              Click Email or Number to copy.
            </div>
          )}
        </div>

        {/* Mobile Menu Button */}
        <button className="md:hidden" onClick={() => setIsOpen(!isOpen)}>
          {isOpen ? (
            <LuX className="text-2xl" />
          ) : (
            <LuMenu className="text-2xl" />
          )}
        </button>

        {/* Navigation Links */}
        <ul
          className={`flex-col md:flex md:flex-row absolute md:static bg-[#0b2f2a] z-50 md:bg-transparent transition-all duration-300 ${
            isOpen ? "top-12 left-0 w-full" : "top-[-200px] md:top-0"
          }`}
        >
          {navs.map((currentNav) => (
            <li key={currentNav.id} className="md:mr-4">
              <NavLink
                to={currentNav.slug}
                className={({ isActive }) =>
                  `flex items-center px-3 py-2 rounded-3xl transition duration-200 ${
                    isActive
                      ? "bg-[#0f4f4c] text-white"
                      : "hover:text-white link-nav"
                  }`
                }
              >
                {currentNav.icon}
                <span className="ml-2">{currentNav.name}</span>
              </NavLink>
            </li>
          ))}
        </ul>
      </div>
    </nav>
  );
};

export default Header;
