import HeaderContent from "../headerContent/HeaderContent";
import "./ourGallary.css";

const images = [
  { src: "./f6.jpeg", alt: "Fish 1 in the pond" },
  { src: "./f4.jpeg", alt: "Fish 2 in the pond" },
  { src: "./f5.jpeg", alt: "Fish 2 in the pond" },
  {
    src: "data:image/jpeg;base64,",
    alt: "Fish 2 in the pond",
  },
  { src: "./f1.jpeg", alt: "Fish 2 in the pond" },
  { src: "./f2.jpeg", alt: "Fish 2 in the pond" },
  { src: "./f3.jpeg", alt: "Fish 2 in the pond" },
];

const OurGallary = () => {
  return (
    <div className="py-8 px-4 relative">
      <HeaderContent
        heading="Gallaries"
        content="Some of our memories that we create on the pond."
      />
      <div className="relative overflow-hidden">
        <div className="left-blur absolute"></div>
        <div className="right-blur absolute"></div>
        <div className="top-blur absolute"></div>
        <div className="bottom-blur absolute"></div>
        <div className="all-images-and-videos">
          {images.map((image, index) => (
            <div key={index} className="img-parent-div">
              <img src={image.src} alt={image.alt} className="image" />
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default OurGallary;
